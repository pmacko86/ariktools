...
public class SamplePairAnalysisFrame extends JFrame implements Runnable {
	
	private static final long serialVersionUID = 5518648258482399544L;
	...
	// Constructor
	public SamplePairAnalysisFrame() {
		super("Sample Pair Analysis");
		...
		randomnessButton = new JButton("Analyze the Sample Pairs");
	...
	private class EventHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource() == loadCoverButton) {
				File f = Utils.chooseImage(SamplePairAnalysisFrame.this, "Choose the source image", true);
				if (f == null) return;
				try {
			...
			if (event.getSource() == lowBitsButton) {
				File f = Utils.chooseImage(SamplePairAnalysisFrame.this, "Save the lower bits as an image", false);
				if (f == null) return;
				if (f.exists()) {
					if (!Utils.shouldOverwrite(SamplePairAnalysisFrame.this, f.getName())) return;
				}
				try {
			...
			if (event.getSource() == saveButton) {
				File f = Utils.chooseFile(SamplePairAnalysisFrame.this, "Save the report", false);
				if (f == null) return;
				if (f.exists()) {
					if (!Utils.shouldOverwrite(SamplePairAnalysisFrame.this, f.getName())) return;
				}
	...
	private class AnalysisThread implements Runnable {
		...
		public void run() {
			...
			try {
				...
				SamplePairAnalysis spa = new SamplePairAnalysis(sourceImage);
				spa.setProgressBar(progress);
				spa.setProgressLabel(progressLabel);
				spa.analyze();

				progress.setIndeterminate(true);
				
				report = spa.generateReport(coverFile.getName());
