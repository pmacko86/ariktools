...
public class ColorAnalysisFrame extends JFrame implements Runnable {
	
	private static final long serialVersionUID = 2534084020127433690L;
	...
	public ColorAnalysisFrame() {
		...
		randomnessLabel = new JLabel("Color Structure Report");
		...
		randomnessButton = new JButton("Analyze Color");
	...
	private class EventHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource() == loadCoverButton) {
				File f = Utils.chooseImage(ColorAnalysisFrame.this, "Choose the source image", true);
				if (f == null) return;
				try {
			...
			if (event.getSource() == lowBitsButton) {
				File f = Utils.chooseImage(ColorAnalysisFrame.this, "Save the lower bits as an image", false);
				if (f == null) return;
				if (f.exists()) {
					if (!Utils.shouldOverwrite(ColorAnalysisFrame.this, f.getName())) return;
				}
				try {
			...
			if (event.getSource() == saveButton) {
				File f = Utils.chooseFile(ColorAnalysisFrame.this, "Save the report", false);
				if (f == null) return;
				if (f.exists()) {
					if (!Utils.shouldOverwrite(ColorAnalysisFrame.this, f.getName())) return;
				}
	...
	private class AnalysisThread implements Runnable {
		...
		public void run() {
			...
			try {
				...
				ColorAnalysis ca = new ColorAnalysis(sourceImage);
				ca.setProgressBar(progress);
				ca.setProgressLabel(progressLabel);
				ca.analyze();

				progress.setIndeterminate(true);
				
				report = ca.generateReport(coverFile.getName());
