...
public class AppendBitStream extends BitStream {
	
	private BitStream stream1;
	private BitStream stream2;
	private int size1;
	
	public AppendBitStream(BitStream stream1, BitStream stream2) {
		super();
		this.stream1 = stream1;
		this.stream2 = stream2;
		this.size1 = stream1.size();
		this.size = size1 + stream2.size();
	}
	
	protected byte get(int index) {
		return index < size1 ? stream1.get(index) : stream2.get(index - size1);
	}
